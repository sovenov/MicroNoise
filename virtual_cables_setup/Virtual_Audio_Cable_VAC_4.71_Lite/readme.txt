...
In case of downloading a ZIP archive, please unpack the distribution
package into an empty folder on a local drive, making sure that local
folder path does not contain special characters like "#", "&" or similar.
Unpacking the package, preserve folder structure (after unpacking,
destination folder should contain subfolders "x86", "x64", "arm64" and
"tools").

After unpacking, run the installer (setup.exe). On ARM64 systems, this
executable file may not be able to run; in such case, run setup64a.exe.

To avoid uncorrectable signal distortions, it is better to check your
system's real-time performance using LatenyMon:

http://www.resplendence.com/latencymon

In your system is considered not suitable for real-time audio
processing, try some tuning steps described here:
